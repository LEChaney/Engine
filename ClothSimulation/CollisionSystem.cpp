...
void CollisionSystem::CapsuleCollision(Entity & clothEntity, Entity & capsuleEntity)
{
	...
	for (ClothNode& clothNode : clothEntity.cloth.clothNodes) {
		float distance;
		glm::vec3 closestPointOnLine;

		PointLineDistance(clothNode.pointMass.getPosition(), capsuleStart, capsuleEnd, distance, closestPointOnLine);

		if (distance < capsuleRadius + 0.04f)
		{
			glm::vec3 pushValue = clothNode.pointMass.getPosition() - closestPointOnLine;
			pushValue = glm::normalize(pushValue) * (capsuleRadius + 0.04f);
			clothNode.pointMass.setPosition(closestPointOnLine + pushValue);
		}
	}
}
...
{
	for (ClothNode& clothNode : clothEntity.cloth.clothNodes) {
		glm::vec3 pointPosition = clothNode.pointMass.getPosition();
		glm::vec3 groundPosition = groundEntity.transform.position;
		if (pointPosition.y < groundPosition.y + 0.04f) {
			clothNode.pointMass.setPosition({ pointPosition.x, groundPosition.y + 0.04f,pointPosition.z });
		}
	}
}
...
void CollisionSystem::PyramidCollision(Entity & clothEntity, Entity & pyramidEntity)
{
	...
	for (size_t i = 0; i < triIndices.size(); i += 3){
		float pointTriangleDistance = glm::length(clothNodes.at(triIndices.at(i)).pointMass.getPosition() - pyramidEntity.transform.position);
		if (pointTriangleDistance < glm::length(pyramidEntity.transform.scale)) {															// Works due to the triangle being 1 unit size
			...
			for (size_t j = 0; j < pyramidIndicies.size(); j += 3) {
				...
				triangle1.push_back(clothNodes.at(triIndices.at(i)).pointMass.getPosition());
				triangle1.push_back(clothNodes.at(triIndices.at(i + 1)).pointMass.getPosition());
				triangle1.push_back(clothNodes.at(triIndices.at(i + 2)).pointMass.getPosition());
				// Triangle 2
				...
				if (intersecting && ! coplanar) {
					...
					for (size_t k = 0; k < penetratingPoints.size(); ++k)
					{
						...
						PointMass& pointToMove = clothEntity.cloth.clothNodes.at(triIndices.at(i + indexes.at(k))).pointMass;
